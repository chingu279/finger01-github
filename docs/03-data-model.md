...
| 소스 | 경로 | 난이도 |
|---|---|---|
| **Apple Health** | 건강 앱 → 프로필 → 모든 건강 데이터 내보내기 → `export.xml` | **구현됨** — `./health import` |
...
### Apple Health 적재에서 실제로 걸리는 것들
`src/health/ingest.py` 가 처리하는 함정들이다. 새 어댑터를 붙일 때도 같은 목록을 본다.

| 함정 | 잘못하면 | 처리 |
|---|---|---|
| HRV 가 SDNN 뿐 | rMSSD 칸에 넣으면 기기 교체 시 베이스라인이 조용히 망가짐 | `vitals.hrv_sdnn_ms` 로 분리, 준비도는 하나만 사용 |
| 걸음이 아이폰·워치에서 이중 기록 | 걸음이 두 배 | 출처별로 합산 후 **최댓값 하나만** 사용 |
| SpO₂ 가 `unit="%"` 에 `0.97` | 트리아지가 매일 응급 발령 | 1.0 이하면 ×100 |
| 체중이 lb | 70kg 이 154kg | ×0.45359237 |
| 수면이 자정을 넘음 | 하루씩 밀린 수면 | **기상한 날**에 귀속 |
| 낮잠이 밤잠에 섞임 | 총 수면 부풀림, 수면부채 과소평가 | 세션 간격 3시간 · 60분 미만 제외 |
| 오작동 값 (심박 410) | 28일 베이스라인이 통째로 밀림 | 저장 거부 + 보고 |
| 값이 전부 거부된 날 | 빈 파일이 연속 기록일을 부풀려 게이트를 거짓 통과시킴 | 빈 레코드는 파일을 만들지 않음 |
| 파일이 수백 MB | 메모리 초과 | `iterparse` 스트리밍 + 즉시 해제 |
